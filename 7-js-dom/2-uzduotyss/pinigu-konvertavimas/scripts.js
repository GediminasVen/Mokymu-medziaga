...
document.querySelector('.calculate').addEventListener('click', () => {
    ...
    let currencyRate = currencies.find(currency => currency.id === fromSelectValue)
        .rates.find(rate => rate.currencyId === toSelectValue).rate;
    ...
    let changedCurrency = (currencyFromValue * currencyRate).toFixed(2);
    let fullResult = `${currencyFromValue} ${fromName} yra ${changedCurrency} ${toName}`;
    let rateResult = `Santykis: 1 ${fromName} = ${currencyRate} ${toName}`;

    document.getElementById('conversion-result').innerHTML = fullResult;
    document.getElementById('exchange-rate').innerHTML = rateResult;
