...
function refreshBasket() {
  ...
  finalOrderPrice.innerHTML =
    countFinalOrderPrice(products).toFixed(2) + " eur";
}
...
function onRemoveButtonClick(id) {
    ...
    if (currentProduct.quantity > 1) {
      ...
      alert("Quantity nerabotina zemiau nei 1");
    }
  }
    ...
    products = products.map((product) => {
      if (product.id === id) {
        return {
          ...
          finalPrice: countFinalPrice(product.price, product.quantity + 1),
        };
      }
      return product;
    });
